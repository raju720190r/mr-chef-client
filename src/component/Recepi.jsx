        ...
        <div className='mb-10'>
            {
                recipe?.ingredients.map((data, index) =>
                    <p className='text-orange-600 my-0' key={index}> {index + 1} . {data}</p>
                )
            }
        </div>
        <div className='flex space-x-2 items-center justify-between '>
            <div className='flex'><Rating style={{ maxWidth: 100 }} value={recipe.ratting} readOnly />
            <span>
                {recipe.ratting}
            </span></div>
            <div><button disabled={!favaroite} onClick={handleFavaroute}> <FaHeart className={  favaroite? 'w-7 h-7 text-zinc-300 shadow-xl': 'w-5 h-5 text-red-800 shadow-xl' } /></button></div>
        </div>
    </Card>
    );
};
